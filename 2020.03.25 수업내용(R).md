...
test1 <-function(p){   #일반적으로 에러가 뜨면 에러 후의 명령은 수행하지 않음.
  cat("난 수행함\n")       #p라는 매개변수가 사용되지 않으면 전달하지 않아도 에러가 뜨지 않고 무시된다.
  testError(-1)
  cat("나 수행할 까요? \n")
}
test1()

#try()    에러가 떠도 그 다음 명령까지 모두 수행함.
test2 <- function(p){
  cat("난 수행함\n")
  try(testError(-1))
  cat("나 수행할 까요? \n")
...
#Sys.sleep(초시간) 함수     #프로그램이 대기상태로 유지하도록 하는 함수.
testSleep <- function(x) {     #초단위로 지정한다. 밀리초 아님.
  for(data in 6:10) {       
    cat(data,"\n")
    if(x)
      Sys.sleep(1)
